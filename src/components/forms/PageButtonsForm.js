...
export default function PageButtonsForm({ user, page }) {
  ...
  function addButtonToProfile(button) {
    setActiveButtons((prev) => {
      if (prev.find((b) => b.key === button.key)) return prev;
      return [...prev, button];
    });
  }
  ...
  function removeButton({ key }) {
    setActiveButtons((prev) => prev.filter((b) => b.key !== key));
  }
    ...
    <SectionBox>
      <form action={saveButtons}>
        ...
        <input
          type="hidden"
          name="_buttonsOrder"
          value={activeButtons.map((b) => b.key).join(",")}
        />
          ...
          {activeButtons.map((b) => (
            <div key={b.key} className="mb-4 md:flex items-center">
              <div className="w-56 flex h-full text-gray-700 p-2 gap-2 items-center">
                <FontAwesomeIcon
                  icon={faGripLines}
                  className="handle cursor-pointer text-gray-400 p-2"
                />
                <FontAwesomeIcon icon={b.icon} />
                <span>{upperFirst(b.label)}:</span>
              </div>
